...
Next, we want to add error bars to each bar to depict the the standard deviation from the mean. This time, we need to add a new function to our figure code called ```geom_errorbar()```. Within that function, you again want to pass information to the function using the ```aes()``` call: ```ymmin``` and ```ymax```. These will tell ```ggplot()``` the minimum and maximum values for your error bars. To create standard deviation error bars, we will need to either add (```ymax```) or subtract (```ymin```) the standard deviation (```sd_Sepal.Length```) from the ```mean_Sepal.Length``` like shown below:
...
**This R assignment will be due by the the start of next lab**. You will submit both your R markdown code and the HTML or Word output of your work. This assignment can be completed with other members from your lab section but if you work with others, please include their names on your submission.   
...
1. **Create a R markdown document that will knit as a Word document.** You will submit both the R markdown code and the HTML or Word document for this assignment so this will be assessed as a part of your submission. 
